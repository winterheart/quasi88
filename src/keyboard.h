...
enum {
  ...
  KEY88_INVALID = 0,
  /* 1~31 are reserved for special function control. */

  /* Definition for character keys (matches ASCII code) */
  ...
  /* Definitions for numeric keypad characters */
  ...
  /* Definitions for function keys for menus */
  ...
  /* Definitions for late function keys */
  ...
  /* Definitions for late special keys */
  ...
  /* 248~255 reserved for the system */
...
#define KEY88_PRESS(code) key_scan[keyport[(code)].port] &= ~keyport[(code)].mask

#define KEY88_RELEASE(code) key_scan[keyport[(code)].port] |= keyport[(code)].mask

#define KEY88_TOGGLE(code) key_scan[keyport[(code)].port] ^= keyport[(code)].mask

#define IS_KEY88_PRESS(code) (~key_scan[keyport[(code)].port] & keyport[(code)].mask)

#define IS_KEY88_RELEASE(code) (key_scan[keyport[(code)].port] & keyport[(code)].mask)
