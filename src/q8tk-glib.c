...
/* プロトタイプ                       */
/********************************************************/
static void q8gr_putchar(int x, int y, int fg, int bg, int reverse, int underline, int c);
...
#define CHECK_MASK_X_FOR(x)                                                                                            \
  if ((x) < screen_mask_x0)                                                                                            \
    continue;                                                                                                          \
  else if ((x) >= screen_mask_x1)                                                                                      \
  break
#define CHECK_MASK_Y_FOR(y)                                                                                            \
  if ((y) < screen_mask_y0)                                                                                            \
    continue;                                                                                                          \
  else if ((y) >= screen_mask_y1)                                                                                      \
  break
...
void q8gr_draw_window(int x, int y, int sx, int sy, int shadow_type, void *p) {
...
void q8gr_draw_frame(int x, int y, int sx, int sy, int shadow_type, int code, const char *str, void *p) {
...
void q8gr_draw_scrolled_window(int x, int y, int sx, int sy, int shadow_type, void *p) {
  draw_normal_box(x, y, sx, sy, shadow_type);
...
void q8gr_draw_check_button(int x, int y, int condition, void *p) {
  ...
  if (p) {
    ...
    if (((Q8tkWidget *)p)->child && ((Q8tkWidget *)p)->child->type == Q8TK_TYPE_LABEL &&
        ((Q8tkWidget *)p)->child->visible && ((Q8tkWidget *)p)->child->sensitive && ((Q8tkWidget *)p)->child->name) {

      int len = q8gr_strlen(((Q8tkWidget *)p)->child->code, ((Q8tkWidget *)p)->child->name);
...
void q8gr_draw_radio_button(int x, int y, int condition, void *p) {
  ...
  q8gr_putc(x + 1, y, fg, bg, ((condition == Q8TK_BUTTON_OFF) ? Q8GR_B_OFF : Q8GR_B_ON));
 ...
 *        ←────sx────→
 *----------------------------------------------------------------------*/
void q8gr_draw_notebook(int x, int y, int sx, int sy, Q8tkWidget *notebook, void *p) {
...
void q8gr_draw_notepage(int code, const char *tag, int select_flag, int active_flag, Q8tkWidget *notebook, void *p) {
  int i, len;
  int fg = (p) ? Q8GR_PALETTE_FOREGROUND : Q8GR_PALETTE_WHITE;
  int bg = Q8GR_PALETTE_BACKGROUND;
...
void q8gr_draw_vseparator(int x, int y, int height) {
  ...
  if (!CHECK_MASK_X(x)) {
    for (j = y; j < y + height; j++) {
      CHECK_MASK_Y_FOR(j);
      q8gr_putc(x, j, Q8GR_PALETTE_FOREGROUND, Q8GR_PALETTE_BACKGROUND, Q8GR_G_I);
    }
  }
}
void q8gr_draw_hseparator(int x, int y, int width) {
  ...
  if (!CHECK_MASK_Y(y)) {
    for (i = x; i < x + width; i++) {
      CHECK_MASK_X_FOR(i);
      q8gr_putc(i, y, Q8GR_PALETTE_FOREGROUND, Q8GR_PALETTE_BACKGROUND, Q8GR_G__);
    }
  }
}
 ...
 *
 *----------------------------------------------------------------------*/
void q8gr_draw_entry(int x, int y, int width, int code, const char *text, int disp_pos, int cursor_pos, void *p) {
 ...
 *        ←─width →
 *----------------------------------------------------------------------*/
void q8gr_draw_list_item(int x, int y, int width, int active, int reverse, int underline, int code, const char *text,
                         void *p) {
  int fg = (active) ? Q8GR_PALETTE_FONT_FG : Q8GR_PALETTE_FOREGROUND;
  ...
  q8gr_strings(x, y, fg, bg, reverse, underline, -1, code, text, 0, width);
...
void q8gr_draw_hscale(int x, int y, Q8Adjust *adj, int active, int draw_value, int value_pos, void *p) {
  if (draw_value) {
    int fg = (active) ? Q8GR_PALETTE_FONT_FG : Q8GR_PALETTE_FOREGROUND;
    int bg = Q8GR_PALETTE_BACKGROUND;
    int vx, vy;
...
void q8gr_draw_vscale(int x, int y, Q8Adjust *adj, int active, int draw_value, int value_pos, void *p) {
  if (draw_value) {
    int fg = (active) ? Q8GR_PALETTE_FONT_FG : Q8GR_PALETTE_FOREGROUND;
    int bg = Q8GR_PALETTE_BACKGROUND;
    int vx, vy;
...
byte q8gr_logo[Q8GR_LOGO_W * Q8GR_LOGO_H * 16] = {
    ...
    ,
};

int q8gr_draw_label(int x, int y, int fg, int bg, int reverse, int underline, int code, const char *str, void *p) {
  if (p == NULL)
    fg = Q8GR_PALETTE_WHITE;
