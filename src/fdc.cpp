 ...
 */
static struct {
  uint8_t c;       /* セクタ ID の C   */
  uint8_t h;       /* セクタ ID の H   */
  uint8_t r;       /* セクタ ID の R   */
  uint8_t n;       /* セクタ ID の N   */
  uint8_t density; /* セクタの記録密度 */
  uint8_t deleted; /* DELETED DATA フラグ   */
  uint8_t status;  /* PC98 BIOSのステータス*/
  uint8_t padding;
...
static struct {
  ...
  uint8_t read;   /* DATA  for  FDC->MAIN */
  uint8_t write;  /* DATA  for  FDC<-MAIN */
  uint8_t TC;     /* TC (1 or 0 )     */
  ...
  uint8_t sc;             /* SC  セクタ数     */
  uint8_t stp;            /* STP 間隔       */
  uint8_t ncn[MAX_DRIVE]; /* NCN 位置 (4台分) */
  uint8_t pcn[MAX_DRIVE]; /* PCN 位置 (4台分) */
  uint8_t st0;            /* ST0          */
  uint8_t st1;            /* ST1          */
  uint8_t st2;            /* ST2          */
  uint8_t st3;            /* ST3          */

  uint8_t c0;        /* C-Phase 1バイト目    */
  uint8_t c1;        /* C-Phase 2バイト目    */
  uint8_t cn;        /* C-Phase NCN      */
  uint8_t s0;        /* C-Phase SPECIFY  */
  uint8_t s1;        /* C-Phase SPECIFY  */
  uint8_t r0;        /* R-Phase ST0      */
  uint8_t r1;        /* R-Phase PCN      */
  uint8_t intr_unit; /* 割込発生元ユニット  */
...
#define sector_density_mismatch()                                                                                      \
  (((sec_buf.density == DISK_DENSITY_SINGLE) && (fdc.mf)) || ((sec_buf.density == DISK_DENSITY_DOUBLE) && (!fdc.mf)))

#define idr_match() (fdc.c == sec_buf.c && fdc.h == sec_buf.h && fdc.r == sec_buf.r && fdc.n == sec_buf.n)
...
static const char *cmd_name[] = {
    "WAIT ------------------", "READ DATA -------------", "READ DELETED DATA -----", "READ DIAGNOSTIC -------",
    "READ ID ---------------", "WRITE DATA ------------", "WRITE DELETED DATA ----", "WRITE ID --------------",
    "SCAN EQUAL ------------", "SCAN LOW OR EQUAL -----", "SCAN HIGH OR EQUAL ----", "SEEK ------------------",
    "RECALIBRATE -----------", "SENSE INT STATUS ------", "SENSE DEVIC STATUS ----", "SPECIFY ---------------",
    "INVALID ---------------",
  ...
  if (fdc_break_flag) {
    for (i = 0; i < NR_BP; i++) {
      if (break_point_fdc[i].type == nStatus && break_point_fdc[i].drive == nDrive + 1 &&
        ...
        if (break_point_fdc[i].sector == nSector + 1 || break_point_fdc[i].sector < 0) {
          printf("*** Break at D:%d T:%d S:%d *** ", nDrive + 1, nTrack, nSector + 1);
          switch (nStatus) {
          case BP_READ:
            printf("( Read )\n");
            break;
 ...
 */
static int fill_sector_gap(int ptr, int drv, uint8_t fdc_mf);
  ...
  if (osd_fseek(drive[drv].fp, drive[drv].disk_top + DISK_TRACK + trk * 4, SEEK_SET) == 0) {
    ...
    if (osd_fread(&r, sizeof(r), 1, drive[drv].fp) == 1) {
      ...
      if (track_top != 0) {
        ...
        drive[drv].track_top = drive[drv].sec_pos = drive[drv].disk_top;
        drive[drv].sec_nr = -1;
      }
    } else
      error = 1;
...
static int disk_now_sec(int drv) {
  ...
  if (osd_fseek(drive[drv].fp, drive[drv].sec_pos, SEEK_SET) == 0) {
    if (osd_fread(c, sizeof(uint8_t), 16, drive[drv].fp) == 16) {
      sec_buf.c = c[DISK_C];
      sec_buf.h = c[DISK_H];
      sec_buf.r = c[DISK_R];
      sec_buf.n = c[DISK_N];
...
/*  void    fdc_write( uint8_t data )  ……… OUT A,(0FBH)      */
/*  uint8_t    fdc_read( void )    ……… IN  A,(0FBH)      */
/*  uint8_t    fdc_status( void )  ……… IN  A,(0FAH)      */
...
static int fdc_check_unit() {
  ...
  if (fdc.command == WRITE_ID || fdc.command == WRITE_DATA || fdc.command == WRITE_DELETED_DATA) {
  ...
  if (fdc.command == READ_DIAGNOSTIC || /* READ DIAGNOSTIC または        */
      fdc.command == WRITE_ID) {        /* WRITE ID        の場合        */
                                        /*    トラック先頭から処理する為 */
                                        /*    必ずトラック変更 (頭出し)  */
    if (!disk_unformat(drv) && (drive[drv].track & 1) == fdc.hd) {
...
static int fdc_search_id() {
  ...
  if (fdc.command == READ_ID) { /* READ ID の場合 */
    ...
    if (fdc.command == READ_DATA || fdc.command == READ_DELETED_DATA) {
...
static int fdc_read_data() {
  ...
  print_fdc_status(((fdc.command == READ_DIAGNOSTIC) ? BP_DIAG : BP_READ), drv, drive[drv].track, drive[drv].sec);
  ...
  } else if (sec_buf.status == STATUS_DE_DD) { /* DATA CRC err */
    fdc.st0 = ST0_IC_AT | (fdc.hd << 2) | fdc.us;
    fdc.st1 = ST1_DE;
  ...
  while (read_size > 0) { /* -------------------指定サイズ分読み続ける */
    ...
    if (osd_fseek(drive[drv].fp, drive[drv].sec_pos + SZ_DISK_ID, SEEK_SET) == 0) {
      if (osd_fread(&data_buf[ptr], sizeof(uint8_t), size, drive[drv].fp) == (size_t)size) {
        error = 0;
      } else
        error = 1;
    } else
...
static int fdc_write_data() {
  ...
  if (disk_not_exist(drv) ||                          /* ディスク無し */
    ...
    if (fdc_ignore_readonly && /* ReadOnly を無視する場合は正常終了 */
        write_protected && drive[drv].protect != DISK_PROTECT_TRUE) {
  ...
  while (write_size > 0) { /* -------------------指定サイズ分書き続ける */
    ...
    if (write_pos + size <= drive[drv].disk_end) {
      if (osd_fseek(drive[drv].fp, write_pos, SEEK_SET) == 0) {
        if (osd_fwrite(&data_buf[ptr], sizeof(uint8_t), size, drive[drv].fp) == (size_t)size) {
          error = 0;
        } else
          error = 1;
      } else
  ...
  if (osd_fseek(drive[drv].fp, /* ID の、DAM/DDAMを更新 */
    ...
    if (osd_fwrite(&c[0], sizeof(uint8_t), 2, drive[drv].fp) == 2) {
      error = 0;
    } else
      error = 1;
  } else
  ...
  if (fdc.sc == 0) { /* アンフォーマットを作成 */
    ...
    if (format_pos + 16 <= drive[drv].disk_end) {
      if (osd_fseek(drive[drv].fp, format_pos, SEEK_SET) == 0) {
        if (osd_fwrite(id, sizeof(uint8_t), 16, drive[drv].fp) == 16) {
          error = 0;
        } else
          error = 1;
      } else
          ...
          if (osd_fwrite(id, sizeof(uint8_t), 16, drive[drv].fp) == 16) {
            ...
            for (j = 0; j < (1 << (fdc.n & 7)); j++) {
              ...
              if (format_pos + 128 <= drive[drv].disk_end) {
                if (osd_fseek(drive[drv].fp, format_pos, SEEK_SET) == 0) {
                  if (osd_fwrite(data, sizeof(uint8_t), 128, drive[drv].fp) == 128) {
                    format_pos += 128;
                  } else {
                    error = 1;
                    break;
    ...
    *table[fdc.command][fdc.step] = fdc.write; /*データ引取*/
    ...
    if (table[fdc.command][fdc.step]) { /* まだデータが必要な時 */
      ...
      case READ_ID:
        ...
        if (fdc.command == READ_ID) {
          ...
          logfdc("%s sk%d mf%d mt%d us%d hd%d eot=%d gpl=%d dtl=%d\n", cmd_name[fdc.command], fdc.sk, fdc.mf, fdc.mt,
                 fdc.us, fdc.hd, fdc.eot, fdc.gpl, fdc.dtl);
        }
        break;
      ...
      case WRITE_ID:
        ...
        if (fdc.command == WRITE_ID) {
          logfdc("%s mf%d us%d hd%d n%d sc%d gpl%d d%02x\n", cmd_name[fdc.command], fdc.mf, fdc.us, fdc.hd, fdc.n,
                 fdc.sc, fdc.gpl, fdc.d);
        } else {
          logfdc("%s sk%d mf%d mt%d us%d hd%d eot=%d gpl=%d dtl=%d\n", cmd_name[fdc.command], fdc.sk, fdc.mf, fdc.mt,
                 fdc.us, fdc.hd, fdc.eot, fdc.gpl, fdc.dtl);
        }
      ...
      case RECALIBRATE:
        ...
        logfdc("%s us%d %02x (Tr.%02d,%02d=>%02d,%02d)\n", cmd_name[fdc.command], fdc.us, fdc.ncn[fdc.us],
               fdc.pcn[fdc.us] * 2, fdc.pcn[fdc.us] * 2 + 1, fdc.ncn[fdc.us] * 2, fdc.ncn[fdc.us] * 2 + 1);
        break;
      ...
      case SPECIFY:                                               /* SPECIFY系 */
        ...
        logfdc("%s srt%dms hut%dms hlt%dms (%02X %02X)\n", cmd_name[fdc.command], fdc.srt_clk / 4000,
               fdc.hut_clk / 4000, fdc.hlt_clk / 4000, fdc.s0, fdc.s1);
  ...
  if (fdc.step == 0) {
    ...
    case SENSE_DEVICE_STATUS:   /* SENSE DEVICE STATUS    */
      if (fdc.us >= NR_DRIVE) { /* ST3 を生成する         */
        ...
        if (disk_not_exist(fdc.us) || (disk_ex_drv & (1 << fdc.us))) { /* ディスク入れ替えチェック */
                                                                       /* thanks peach! */
          disk_ex_drv ^= (1 << fdc.us);                                /* ドライブ番号のビット反転 */
          fdc.st3 = ((fdc.status & (1 << fdc.us)) ? 0 : ST3_RY) | ((fdc.pcn[fdc.us] == 0) ? ST3_T0 : 0) |
                    ((fdc.hd << 2) & ST3_HD) | (fdc.us & ST3_US);
        } else {
          fdc.st3 = ((fdc.status & (1 << fdc.us)) ? 0 : ST3_RY) |
                    ((drive[fdc.us].protect == DISK_PROTECT_TRUE) ? ST3_WP : 0) |
                    ((fdc.pcn[fdc.us] == 0) ? ST3_T0 : 0) | ST3_TS | ((fdc.hd << 2) & ST3_HD) | (fdc.us & ST3_US);
        }
      }
      logfdc("\t\t\t\t\t\t<st3:%02x>\n", fdc.st3);
      break;
...
static int e_phase_read_end(int interval) {
  ...
  if (fdc.command != READ_DIAGNOSTIC && fdc.st0 & ST0_IC) { /* ID/DATA CRCエラーありの場合  */
...
int fdc_ctrl(int interval) {
      ...
      if ((fdc.status & REQ_MASTER) == 0) { /* コマンド受信した場合   */
        ...
        for (i = 0; i < MAX_DRIVE; i++) { /* シーク完了判定 */
          if (fdc.seek_stat[i] == SEEK_STAT_END) {
            ...
            fdc.seek_wait[i] = 0;
            if (i < NR_DRIVE && disk_not_exist(i) == false && sec_buf.drv == -1) {
  ...
  if (fdc.command == WAIT) { /* 待機中のみ、シーク完了時間を確認 */
    for (i = 0; i < MAX_DRIVE; i++) {
      if (fdc.seek_stat[i] == SEEK_STAT_MOVE && fdc.seek_wait[i] > 0) {
        if (w == -1)
          w = fdc.seek_wait[i];
 ...
 */
INLINE
int calc_gap3_size(int n, uint8_t fdc_mf) {
  int gap3_size;
