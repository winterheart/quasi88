...
/* メモリ読み込み用マクロ */
#define READ_BYTE(mem, addr) ((mem)[addr] & 0xff)

#define READ_WORD(mem, addr) ((READ_BYTE(mem, addr + 1) << 8) + READ_BYTE(mem, addr))

/* メモリ書き込み用マクロ */
#define WRITE_BYTE(mem, addr, data) (mem)[addr] = ((data)&0xff)
...
/* レジスタ書き込み用マクロ */
#define SET_REG(reg, data) ((reg).W = data, (reg).B.h = ((data) >> 8) & 0xff, (reg).B.l = (data)&0xff)
