...
byte main_fetch(word addr) {
  ...
  if (highspeed_mode) {
    ...
    } else if ((highspeed_flag) && (ret_addr == addr || z80main_cpu.state0 >= HS_BASIC_COUNT)) {
      ret_addr = 0xffff;
      /*printf("'%d'\n",z80_state_intchk);*/
...
void main_io_out(byte port, byte data) {
  ...
  case 0x52:
    ...
    if (new_pal.blue != vram_bg_palette.blue || new_pal.red != vram_bg_palette.red ||
  ...
  case 0x53:
    ...
    /* パレット設定 */
  case 0x54:
    if ((data & 0x80) && (misc_ctrl & MISC_CTRL_ANALOG)) { /* アナログモード */
      if ((data & 0x40) == 0) {
  ...
  case 0x5b:
    ...
    if (new_pal.blue != vram_palette[port - 0x54].blue || new_pal.red != vram_palette[port - 0x54].red ||
        new_pal.green != vram_palette[port - 0x54].green) {
...
byte main_io_in(byte port) {
      ...
      do {
        ...
        for (i = 0; i < 0x10; i++) {
          if (i != port && key_scan[i] != 0xff) {
            ...
            if ((i == 0x08 && (mkey | key_scan[i] | 0xf0) != 0xff) || (i != 0x08 && (mkey | key_scan[i]) != 0xff))
              mkey &= key_scan[i];
          }
        }
      } while (mkey_old != mkey);
  ...
  case 0xe3:
    if (linear_ext_ram) { /* 出力値の通りにバンクを割り振った */
      if (use_extram && (ext_ram_bank < use_extram * 4))
        return ext_ram_bank;
      return 0xff;
      ...
      if (use_extram && (ext_ram_bank != 0xff)) {
        if (use_extram <= 4) { /* 128KB*4以下 */
          ...
          if (ext_ram_bank < 0x20) {
            ret = ((ext_ram_bank & 0x18) << 1) | 0x08 | (ext_ram_bank & 0x07);
          } else {
          ...
          /* 設定 08-0Fh, 18-1Fh, 28-2Fh, 38-3Fh とする */
          /* 設定 48-4Fh, 58-5Fh, 68-6Fh, 78-7Fh とする */
          ret = ((ext_ram_bank & 0x38) << 1) | 0x08 | (ext_ram_bank & 0x07);
        }
      }
      /*printf("IN E3 -----> %02X\n",ret);*/
      return ret;
...
static void sio_set_intr_base(void) {
  ...
  if ((fp_si || fp_ti || use_siomouse) && (sio_command & 0x04) && ((sys_ctrl & 0x20) || (sys_ctrl & 0x08))) {
...
static byte sio_in_status(void) {
  ...
  if (sio_command & 0x01) { /* 現在、送信イネーブルの場合 */
    if (((sys_ctrl & 0x20) /*&& sio_serial_writable()*/) || (!(sys_ctrl & 0x20) && sio_tape_writable())) {
      status |= 0x01; /* 送信レディ */
                      /* TxRDY */
    }
  }
...
int tape_writing(void) { return (fp_to && (sio_command & 1) && ((sys_ctrl & 0x28) == 0x08)); }
...
void power_on_ram_init(void) {
  ...
  for (addr = 0; addr < 0x4000; addr += 0x100) {
    ...
    if (((addr & 0x0d00) == 0x0100) || /* x100, x300 */
      ...
      data = 0xff;
    } else if (((addr & 0x0f00) == 0x0500) && ((addr & 0x2000) == 0x0000)) { /* 0500, 1500 */
      data = 0xff;
    } else if (((addr & 0x0f00) == 0x0a00) && ((addr & 0x3000) != 0x0000)) { /* 1a00, 2a00, 3a00 */
      data = 0xff;
    } else {
      data = 0x00;
    }
...
INLINE void check_break_point(int type, word addr, byte data, char *str) {
  ...
  if (quasi88_is_monitor())
    return; /* モニターモード時はスルー */
  for (i = 0; i < NR_BP; i++) {
    if (break_point[BP_MAIN][i].type == type && break_point[BP_MAIN][i].addr == addr) {
      printf("*** Break at %04x *** "
