...
void debuglog_sync(void) { fflush(LOG); }
...
void logz80_target(int debug_flag) { z80_debug_wk = debug_flag; }
...
void profiler_init(void) {
  ...
  if (prof_lap_fp) {
    fprintf(prof_lap_fp, "%-16s%8ld[us]\n", "(vsync)", (long)(1000000.0 / (CONST_VSYNC_FREQ * wait_rate / 100)));
  }
}
  ...
  if (debug_profiler & 2) {
    ...
    for (i = 0; i < PROF_LAPSE_END; i++) {
      ...
      printf("%-16s%5d[times], %4d.%06ld[sec] (ave = %f[sec])\n", prof_label[i], prof_lap[i].count,
             (int)prof_lap[i].all.tv_sec, prof_lap[i].all.tv_usec, d / prof_lap[i].count);
    }
    printf("\n");
  }
...
void profiler_watch_start(void) { gettimeofday(&watch_t0, 0); }
