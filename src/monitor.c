...
static struct {
  ...
  char *help_mes;
    ...
    },
    {
        ...
        "statesave",
        help_statesave,
        "state-save QUASI88kai",
    },
    {
        MONITOR_STATELOAD,
        "stateload",
        help_stateload,
        "state-load QUASI88kai",
    },
    {
        MONITOR_SNAPSHOT,
        "snapshot",
...
static void monitor_search(void) {
  ...
  for (i = 0; i < save_search_size; i++) {
    if (peek_memory(save_search_bank, save_search_addr + i) == save_search_data) {
...
static void monitor_break(void) {
  ...
  if (exist_argv()) {
    ...
    if (exist_argv()) { /* [#<No>] */
      if (argv.val < 0 || argv.val > NR_BP || (action != ARG_CLEAR && argv.val < 1))
        error();
      number = argv.val - 1;
      shift();
    }
...
static void monitor_fbreak(void) {
  ...
  if (show) {
    ...
    if (action == ARG_CLEAR) {
      ...
      printf("set break point - #%d [ %s : D:%d T:%d ", number + 1, s, drive, track);
      if (sector >= 0)
        printf("S:%d ", sector);
      printf("]\n");
    }
...
static void print_text_screen(void) {
  ...
  for (i = 0, end = 0; i < line; i++) {
    for (j = 0; j < width; j++) {
      if (width == 80) {
        text_buf[j] = text_attr_buf[text_attr_flipflop][i * 80 + j] >> 8;
      } else {
        text_buf[j] = text_attr_buf[text_attr_flipflop][i * 80 + j * 2] >> 8;
      }
      if (text_buf[j] == 0)
        text_buf[j] = ' ';
      else
...
static void monitor_set_mem_printf(void) /*** set mem ***/
{
  ...
  if ((misc_ctrl & MISC_CTRL_EVRAM) &&   /* 拡張アクセスモード */
    ...
    rC000 = rF000 = wC000 = wF000 = (memory_bank == 0) ? "VRAM B" : ((memory_bank == 1) ? "VRAM R" : "VRAM G");
  } else {
...
static void monitor_set_palette_printf(void) /*** set palette ***/
{
  ...
  const char *pal = (misc_ctrl & MISC_CTRL_ANALOG) ? "    [      *Analog*]" : "    [     *Digital*]";

  printf("  %-23s %-15sG:R:B = %01X:%01X:%01X\n", "vram_bg_palette", "(OUT:52/54)", vram_bg_palette.green,
         vram_bg_palette.red, vram_bg_palette.blue);

  for (j = 0; j < 8; j++) {
    printf("  %-23s (OUT:%02X)       G:R:B = %01X:%01X:%01X\n",
           (j == 0) ? "vram_palette" : ((j == 2) ? now : ((j == 3) ? pal : "")), j + 0x54, vram_palette[j].green,
...
static void monitor_set_crtc_printf(void) /*** set crtc ***/
{
  ...
  const char *cursortype[] = {"(noblink, underline)", "(blink, underline)", "(noblink, block)", "(blink, block)"};
  ...
  const char *attr[] = {"(mono, sepatare, o)",
                        "(no-attr, x)",
                        "(color, sepatare, o)",
                        "(forbid)",
                        "(mono, mixed, o)",
  ...
  printf("    %-38s%02XH(%d) , %02XH(%d)\n", "Cursor position[X,Y]", crtc_cursor[0], crtc_cursor[0], crtc_cursor[1],
         crtc_cursor[1]);
  ...
  printf("  %-23s %-15s %02X (%X%X%X%X%X%X%X%X)\n", "", "Mode", dmac_mode, (dmac_mode >> 7) & 0x01,
         (dmac_mode >> 6) & 0x01, (dmac_mode >> 5) & 0x01, (dmac_mode >> 4) & 0x01, (dmac_mode >> 3) & 0x01,
         (dmac_mode >> 2) & 0x01, (dmac_mode >> 1) & 0x01, (dmac_mode >> 0) & 0x01);
  printf("  %-23s %-15s %s\n", "  text_display", "",
...
static void monitor_set_pio_printf(void) /*** set pio ***/
{
  ...
  printf("              .exist    %s       \\  /     %s               .exist\n",
         (pio_AB[0][0].exist == PIO_EXIST) ? "*" : "-", (pio_AB[1][0].exist == PIO_EXIST) ? "*" : "-");

  printf("              .data     %02XH      \\/      %02XH             .data\n", (pio_AB[0][0].data),
         (pio_AB[1][0].data));

  printf("                                 /\\\n");

  printf("  pio_AB[0][1].type     %s    ___/  \\___  %s   pio_AB[1][1].type\n",
         (pio_AB[0][1].type == PIO_READ) ? "R" : "W", (pio_AB[1][1].type == PIO_READ) ? "R" : "W");

  printf("              .exist    %s                %s               .exist\n",
         (pio_AB[0][1].exist == PIO_EXIST) ? "*" : "-", (pio_AB[1][1].exist == PIO_EXIST) ? "*" : "-");
  ...
  printf("  pio_C[0][0].type      %s    ___    ___  %s   pio_C[1][0].type\n",
         (pio_C[0][0].type == PIO_READ) ? "R" : "W", (pio_C[1][0].type == PIO_READ) ? "R" : "W");

  printf("             .data      %02XH     \\  /     %02XH            .data\n", (pio_C[0][0].data),
         (pio_C[1][0].data));

  printf("             .cont_f    % 2d       \\/      % 2d             .cont_f\n", (pio_C[0][0].cont_f),
         (pio_C[1][0].cont_f));

  printf("                                 /\\\n");

  printf("  pio_C[0][1].type      %s    ___/  \\___  %s   pio_C[1][1].type\n",
         (pio_C[0][1].type == PIO_READ) ? "R" : "W", (pio_C[1][1].type == PIO_READ) ? "R" : "W");

  printf("             .data      %02XH              %02XH            .data\n", (pio_C[0][1].data), (pio_C[1][1].data));
...
static void monitor_set_volume(int vol) { xmame_cfg_set_mastervolume(vol); }
static void monitor_set_fmmixer(int vol) { xmame_cfg_set_mixer_volume(XMAME_MIXER_FM, vol); }
static void monitor_set_psgmixer(int vol) { xmame_cfg_set_mixer_volume(XMAME_MIXER_PSG, vol); }
static void monitor_set_beepmixer(int vol) { xmame_cfg_set_mixer_volume(XMAME_MIXER_BEEP, vol); }
static void monitor_set_rhythmmixer(int vol) { xmame_cfg_set_mixer_volume(XMAME_MIXER_RHYTHM, vol); }
static void monitor_set_adpcmmixer(int vol) { xmame_cfg_set_mixer_volume(XMAME_MIXER_ADPCM, vol); }
#ifdef USE_FMGEN
static void monitor_set_fmgenmixer(int vol) { xmame_cfg_set_mixer_volume(XMAME_MIXER_FMGEN, vol); }
#endif
static void monitor_set_samplemixer(int vol) { xmame_cfg_set_mixer_volume(XMAME_MIXER_SAMPLE, vol); }
#endif /* USE_SOUND */
...
static void monitor_set_show_printf(int index) /*** set (print) ***/
{
  ...
  MTYPE_numeric_variable:;
    printf("  %-23s %-15s %08XH (%d)\n", monitor_variable[index].var_name, monitor_variable[index].port_mes, val, val);
    break;
...
static void monitor_set(void) {
  ...
  if (block) {
    ...
    for (; index < COUNTOF(monitor_variable); index++) {
      if (monitor_variable_block[set_type].start <= block && block <= monitor_variable_block[set_type].end) {
    ...
    for (i = 0; i < NR_DRIVE; i++) {
      ...
      if (disk_image_exist(i)) {
        ...
        printf("    Selected image No = %d/%d%s : protect = %s : media = %s\n", drive[i].selected_image + 1,
               drive[i].image_nr,
               (drive[i].detect_broken_image) ? " + broken" : ((drive[i].over_image) ? " + alpha " : "         "),
               (drive[i].protect == DISK_PROTECT_TRUE) ? "RO" : "RW",
               (drive[i].type == DISK_TYPE_2HD) ? "2HD" : ((drive[i].type == DISK_TYPE_2DD) ? "2DD" : "2D"));
        printf("    ------------------------------------------\n");
        for (j = 0; j < drive[i].image_nr; j++) {
          printf("    %s% 3d %-17s  %s  %s  %ld\n", (j == drive[i].selected_image) ? "*" : " ", j + 1,
                 ...
                 (drive[i].image[j].type == DISK_TYPE_2D)
                     ...
                     : ((drive[i].image[j].type == DISK_TYPE_2DD)
                            ? "2DD"
                            : ((drive[i].image[j].type == DISK_TYPE_2HD) ? "2HD" : "???")),
  ...
  case ARG_SHOW:
    ...
    while ((result = d88_read_header(fp, offset, c)) == 0) {
      ...
      printf("  % 3d   %-17s  %s  %s  %ld\n", num + 1, c,
             (c[DISK_PROTECT] == DISK_PROTECT_TRUE) ? "RO" : ((c[DISK_PROTECT] == DISK_PROTECT_FALSE) ? "RW" : "??"),
             (c[DISK_TYPE] == DISK_TYPE_2D)
                 ? "2D "
                 : ((c[DISK_TYPE] == DISK_TYPE_2DD) ? "2DD" : ((c[DISK_TYPE] == DISK_TYPE_2HD) ? "2HD" : "???")),
