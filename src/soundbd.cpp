...
enum AdpcmStat { ADPCM_HALT, ADPCM_RECORD, ADPCM_REPLAY, ADPCM_WRITE, ADPCM_READ, EndofAdpcmStat };
...
#else
      if (sound2_data_addr == sound2_stop_addr || sound2_data_addr == 0x3ffff) {
        /* sound2_FLAG_EOS  = sound2_MSK_EOS  ? 0 : 1; */
        sound2_delay |= 0x04;
      }
      sound2_adpcm[sound2_data_addr] = data;
...
byte sound2_in_status(void) {
  ...
  return (sound2_FLAG_PCMBSY << 5) | (sound2_FLAG_ZERO << 4) | (sound2_FLAG_BRDY << 3) | (sound2_FLAG_EOS << 2) |
...
void sound_output_after_stateload(void) {
  ...
  static const int addr_2203[][2] = {
      {0x00, 0x0f}, {0x24, 0x28}, {0x30, 0x3e}, {0x40, 0x4e}, {0x50, 0x5e}, {0x60, 0x6e}, {0x70, 0x7e},
      {0x80, 0x8e}, {0x90, 0x9e}, {0xa0, 0xa2}, {0xa4, 0xa6}, {0xa8, 0xaa}, {0xac, 0xae}, {0xb0, 0xb2},
  };
  static const int addr_2608[][2] = {
      {0x00, 0x0f}, {0x11, 0x12}, {0x18, 0x1d}, {0x22, 0x22}, {0x24, 0x29}, {0x30, 0x3e},
      {0x40, 0x4e}, {0x50, 0x5e}, {0x60, 0x6e}, {0x70, 0x7e}, {0x80, 0x8e}, {0x90, 0x9e},
      {0xa0, 0xa2}, {0xa4, 0xa6}, {0xa8, 0xaa}, {0xac, 0xae}, {0xb0, 0xb2},
      /*{ 0xb4, 0xb6 },*/ /* このレジスタを resume させると音が変 */
  };
  static const int addr_2608_2[][2] = {
      {0x01, 0x10}, {0x30, 0x3e}, {0x40, 0x4e}, {0x50, 0x5e}, {0x60, 0x6e}, {0x70, 0x7e}, {0x80, 0x8e},
      {0x90, 0x9e}, {0xa0, 0xa2}, {0xa4, 0xa6}, {0xa8, 0xaa}, {0xac, 0xae}, {0xb0, 0xb2}, {0xb4, 0xb6},
  };
