...
#define DST_RESTORE_LINE()                                                                                             \
  dst -= CHARA_LINES * 4 * dst_w;                                                                                      \
...
#define DST_NEXT_TOP_CHARA()                                                                                           \
  dst += CHARA_LINES * 4 * dst_w - 1280;                                                                               \
...
#define COPY_8DOT()                                                                                                    \
  memcpy(dst2, dst, sizeof(TYPE) * 16);                                                                                \
  memcpy(dst3, dst, sizeof(TYPE) * 16);                                                                                \
  memcpy(dst4, dst, sizeof(TYPE) * 16);
#define COPY_16DOT()                                                                                                   \
  memcpy(dst2, dst, sizeof(TYPE) * 32);                                                                                \
...
#define DST_V(idx, c)                                                                                                  \
  dstbuf[2 * (idx)] = dstbuf[2 * (idx) + 1] = c;                                                                       \
  dstbuf3[2 * (idx)] = dstbuf3[2 * (idx) + 1] = BLACK;
#define DST_T(idx)                                                                                                     \
  dstbuf[2 * (idx)] = dstbuf[2 * (idx) + 1] = tcol;                                                                    \
  dstbuf3[2 * (idx)] = dstbuf3[2 * (idx) + 1] = tcol;
#define DST_B(idx)                                                                                                     \
  dstbuf[2 * (idx)] = dstbuf[2 * (idx) + 1] = BLACK;                                                                   \
  dstbuf3[2 * (idx)] = dstbuf3[2 * (idx) + 1] = BLACK;
...
#define DST_V(idx, c)                                                                                                  \
  dst[2 * (idx)] = dst[2 * (idx) + 1] = c;                                                                             \
  dst3[2 * (idx)] = dst3[2 * (idx) + 1] = BLACK;
#define DST_T(idx)                                                                                                     \
  dst[2 * (idx)] = dst[2 * (idx) + 1] = tcol;                                                                          \
  dst3[2 * (idx)] = dst3[2 * (idx) + 1] = tcol;
#define DST_B(idx)                                                                                                     \
  dst[2 * (idx)] = dst[2 * (idx) + 1] = BLACK;                                                                         \
  dst3[2 * (idx)] = dst3[2 * (idx) + 1] = BLACK;
