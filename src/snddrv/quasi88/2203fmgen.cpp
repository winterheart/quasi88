...
static void *fmgen2203_start(int sndindex, int clock, const void *config) {
  ...
  if (info->opn->Init(clock, Machine->sample_rate ? Machine->sample_rate : 44100)) {
    return info;
  }
...
void fmgen2203_get_info(void *token, UINT32 state, sndinfo *info) {
  ...
  case SNDINFO_PTR_SET_INFO:
    info->set_info = fmgen2203_set_info;
    break;
  case SNDINFO_PTR_START:
    info->start = fmgen2203_start;
    break;
  case SNDINFO_PTR_STOP:
    info->stop = fmgen2203_stop;
    break;
  case SNDINFO_PTR_RESET:
    info->reset = fmgen2203_reset;
    break;
