...
void imagefile_all_open(int stateload) {
  ...
  if (file_disk[0][0] && /* ドライブ1,2 ともイメージ指定済みの場合 */
    ...
    err1 = disk_insert(DRIVE_2, file_disk[1], (image_disk[1] < 0) ? 0 : image_disk[1], readonly_disk[1]);
  }
...
static char *assemble_filename(const char *imagename, const char *basedir, const char *suffix) {
  ...
  if (osd_path_split(imagename, buf, file, OSD_MAX_FILENAME)) {
    ...
    if (len >= 4) {
      if (strcmp(&file[len - 4], ".d88") == 0 || strcmp(&file[len - 4], ".D88") == 0 ||
          strcmp(&file[len - 4], ".t88") == 0 || strcmp(&file[len - 4], ".T88") == 0 ||
          strcmp(&file[len - 4], ".cmt") == 0 || strcmp(&file[len - 4], ".CMT") == 0) {
        file[len - 4] = '\0';
      }
    }
