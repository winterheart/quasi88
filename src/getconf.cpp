...
static const T_CONFIG_TABLE option_table[] = {
    ...
    {6, "romboot", X_FIX, &boot_from_rom, BOOT_ROM, 0, nullptr, nullptr},
    {6, "diskboot", X_FIX, &boot_from_rom, BOOT_DISK, 0, nullptr, nullptr},
    ...
    {163, "utf8", X_FIX, &file_coding, FILE_CODING_UTF8, 0, nullptr, nullptr},
    {163, "sjis", X_FIX, &file_coding, FILE_CODING_SJIS, 0, nullptr, nullptr},
    {163, "euc", X_FIX, &file_coding, FILE_CODING_EUC, 0, nullptr, nullptr},
...
static int check_option(char *opt1, char *opt2, int priority, const T_CONFIG_TABLE *osd_options,
 ...
 *--------------------------------------------------------------------------*/
static int get_option(int argc, char *argv[], int priority, const T_CONFIG_TABLE *osd_options,
                      const T_CONFIG_TABLE *sound_options) {
  int i, j;
  int drive = DRIVE_1;
  char *p;
...
static int get_config_file(OSD_FILE *fp, int priority, const T_CONFIG_TABLE *osd_options,
                           const T_CONFIG_TABLE *sound_options) {
  int result;
...
static int save_key_sub(int type, const struct T_CONFIG_TABLE *op, char opt_arg[255]) {
...
static int save_cur(const struct T_CONFIG_TABLE *op, char opt_arg[255]) { return save_key_sub(1, op, opt_arg); }
static int save_mou(const struct T_CONFIG_TABLE *op, char opt_arg[255]) { return save_key_sub(2, op, opt_arg); }
static int save_joy(const struct T_CONFIG_TABLE *op, char opt_arg[255]) { return save_key_sub(3, op, opt_arg); }
...
int config_save(const char *fname) {
  ...
  if (fp) {
    ...
    if (fname_bak) {
      ...
      if ((flen > slen) && my_strcmp(&fname_bak[flen - slen], CONFIG_SUFFIX) == 0) {
  ...
  if (backup_ok) { /* OKなら設定ファイル新規作成*/
    ...
    while (osd_fgets(line, MAX_RCFILE_LINE, fp_bak)) {
      ...
      if ((flen - 1 > slen) && strncmp(&line[flen - 1 - slen], AUTO_SAVE_COMMENT, slen) == 0) {
        /* skip */
      } else {
        osd_fputs(line, fp);
  ...
  /* ここから追記開始 */
  sprintf(buf, "# The following settings are written by %s", Q_TITLE " ver " Q_VERSION);
  sprintf(line, "%-70s %s\n", buf, AUTO_SAVE_COMMENT);
  osd_fputs(line, fp);
