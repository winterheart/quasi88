...
static int select_file(int for_read, const char *title, const char *filter, const char *def_ext, int flag,
...
static void update_drive() {
  ...
  for (drv = 0; drv < NR_DRIVE; drv++) {
    ...
    if (disk_image_exist(drv)) {
      ...
      for (i = 0; i < MIN(disk_image_num(drv), 9); i++) {
        ...
        InsertMenu(g_hMenu, tag, MF_BYCOMMAND | MFT_STRING, uItem, buf);
      }
    ...
    CheckMenuRadioItem(g_hMenu, base, tag, uItem, MF_BYCOMMAND);
  }
  ...
  EnableMenuItem(g_hMenu, M_DRV_UNSET, MF_BYCOMMAND | ((has_image) ? MFS_ENABLED : MFS_GRAYED));
...
static void update_misc_cload() {
  ...
  uItem = M_MISC_CLOAD_S;
  CheckMenuItem(g_hMenu, uItem, MF_BYCOMMAND | ((s) ? MFS_CHECKED : MFS_UNCHECKED));

  /* テープありなら unset を表示、なしなら隠す */
  uItem = M_MISC_CLOAD_U;
  EnableMenuItem(g_hMenu, uItem, MF_BYCOMMAND | ((s) ? MFS_ENABLED : MFS_GRAYED));
...
static void update_misc_record() {
  ...
  uItem = M_MISC_RECORD;
  CheckMenuItem(g_hMenu, uItem, MF_BYCOMMAND | (i ? MFS_CHECKED : MFS_UNCHECKED));
}
...
static void menubar_item_setup() {
  ...
  CheckMenuRadioItem(g_hMenu, M_SYS_MODE_V2, M_SYS_MODE_N, uItem, MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(g_hMenu, M_SYS_MODE_4MH, M_SYS_MODE_8MH, uItem, MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(g_hMenu, M_SYS_MODE_SB, M_SYS_MODE_SB2, uItem, MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(g_hMenu, M_SET_SPD_25, M_SET_SPD_400, uItem, MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(g_hMenu, M_SET_SUB_SOME, M_SET_SUB_ALL, uItem, MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(g_hMenu, M_SET_REF_60, M_SET_REF_15, uItem, MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(g_hMenu, M_SET_INT_NO, M_SET_INT_YES, uItem, MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(g_hMenu, M_SET_SIZ_FULL, M_SET_SIZ_DOUBLE, uItem, MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(g_hMenu, M_SET_MO_NO, M_SET_MO_JOY, uItem, MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(g_hMenu, M_SET_CUR_DEF, M_SET_CUR_TEN, uItem, MF_BYCOMMAND);
  ...
  if (xmame_has_sound()) {
    ...
    CheckMenuRadioItem(g_hMenu, M_SET_FM_MAME, M_SET_FM_FMGEN, uItem, MF_BYCOMMAND);
#endif
    ...
    CheckMenuRadioItem(g_hMenu, M_SET_FRQ_48, M_SET_FRQ_11, uItem, MF_BYCOMMAND);
    ...
    if (xmame_has_audiodevice()) {
      ...
      CheckMenuRadioItem(g_hMenu, M_SET_BUF_800, M_SET_BUF_100, uItem, MF_BYCOMMAND);
    } else {
#ifdef USE_FMGEN
...
int menubar_event(int id) {
  ...
  case M_SET_SIZ_HALF:
    ...
    break;
#ifdef SUPPORT_DOUBLE
  case M_SET_SIZ_DOUBLE:
    f_set_size(id, SCREEN_SIZE_DOUBLE);
    break;
#endif

  case M_SET_FULLSCREEN:
    f_set_fullscreen(id);
    break;
...
static void f_sys_menu() { quasi88_menu(); }

static void f_sys_save() { config_save(nullptr); }

static void f_sys_exit() { quasi88_quit(); }
...
static void f_set_nowait(UINT uItem) {
  ...
  CheckMenuItem(g_hMenu, uItem, MF_BYCOMMAND | (active ? MFS_CHECKED : MFS_UNCHECKED));
...
static void f_drv_chg(int data) {
  ...
  result = select_file(TRUE, headline, "d88(*.d88)\0*.d88\0All files(*.*)\0*.*\0\0", "d88",
...
static void f_drv_drv1(UINT uItem, int data) {
  ...
  CheckMenuRadioItem(g_hMenu, M_DRV_DRV1_1, M_DRV_DRV1_NO, uItem, MF_BYCOMMAND);
  {
    if ((int)data < 0) {
...
static void f_drv_drv2(UINT uItem, int data) {
  ...
  CheckMenuRadioItem(g_hMenu, M_DRV_DRV2_1, M_DRV_DRV2_NO, uItem, MF_BYCOMMAND);
  {
    if ((int)data < 0) {
...
static void f_misc_cload_s() {
  ...
  result =
      select_file(TRUE, "Open Tape-Image-File for LOAD", "*.t88;*.cmt\0*.t88;*.cmt\0All files(*.*)\0*.*\0\0", "cmt",
                  OFN_FILEMUSTEXIST |   /* 既存ファイル限定 */
...
static void f_misc_csave_s() {
  ...
  result = select_file(FALSE, "Open Tape-Image-File for SAVE (append)", "*.cmt\0*.cmt\0All files(*.*)\0*.*\0\0", "cmt",
