...
unsigned char ByteReader(byte *buf, size_t nOffs) { return *(buf + nOffs); }

void ByteWriter(byte *buf, size_t nOffs, unsigned char nVal) { *(buf + nOffs) = nVal; }

unsigned char DummyReader(size_t nOffs) { return 0; }

void DummyWriter(size_t nOffs, unsigned char nVal) { return; }

unsigned char MainRAMReader(size_t nOffs) { return ByteReader(main_ram, nOffs); }

void MainRAMWriter(size_t nOffs, unsigned char nVal) { ByteWriter(main_ram, nOffs, nVal); }

#if RA_ENABLE_TVRAM
unsigned char TVRAMReader(size_t nOffs) { return ByteReader(main_high_ram, nOffs); }

void TVRAMWriter(size_t nOffs, unsigned char nVal) { ByteWriter(main_high_ram, nOffs, nVal); }
#endif

#if RA_ENABLE_GVRAM
unsigned char GVRAMReader(size_t nOffs) { return ByteReader(main_vram[nOffs >> 14], nOffs % 0x4000); }

void GVRAMWriter(size_t nOffs, unsigned char nVal) { ByteWriter(main_vram[nOffs >> 14], nOffs % 0x4000, nVal); }
#endif

#if RA_ENABLE_EXTRAM
unsigned char ExtRAMReader(size_t nOffs) { return ByteReader((byte *)ext_ram, nOffs); }

void ExtRAMWriter(size_t nOffs, unsigned char nVal) { ByteWriter((byte *)ext_ram, nOffs, nVal); }
#endif
...
bool GameIsActive() { return loaded_title != NULL; }

void CauseUnpause() { quasi88_exec(); }

void CausePause() { quasi88_pause(); }
...
void LoadROM(const char *sFullPath) { quasi88_disk_insert_all(sFullPath, TRUE); }

void RA_InitShared() {
  RA_InstallSharedFunctions(&GameIsActive, &CauseUnpause, &CausePause, &RebuildMenu, &GetEstimatedGameTitle,
                            &ResetEmulation, &LoadROM);
}
...
#define RA_RELOAD_MULTI_DISK                                                                                           \
  FALSE /* ディスクを切り替えるときは同じタイトルが検出されても、                           \
          実績システムを再初期化する */
int RA_PrepareLoadNewRom(const char *file_name, int file_type) {
  ...
  should_activate =
      ...
      : loaded_title != NULL && loaded_title->title_id > 0 && loaded_title->title_id == loading_file.title_id ? false
                                                                                                              : true;
#endif
...
void RA_AbortLoadNewRom() { free_file_info(&loading_file); }
...
void RA_ToggleLoad(int enabled) { enable_loading = enabled; }

int RA_HandleMenuEvent(int id) {
  if (LOWORD(id) >= IDM_RA_MENUSTART && LOWORD(id) < IDM_RA_MENUEND) {
...
void RA_RenderOverlayFrame(HDC hdc) {
  ...
  RA_UpdateRenderOverlay(hdc, &input, delta_time, &window_size, (bool)quasi88_cfg_now_fullscreen(),
                         (bool)quasi88_is_pause());
