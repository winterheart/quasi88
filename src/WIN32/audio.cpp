...
static const T_CONFIG_TABLE xmame_options[] = {
    ...
    {354, "volume", X_INV, &invalid_arg, 0, 0, nullptr, nullptr},
    ...
    {364, "pcmbufsize", X_INT, &g_pcm_bufsize, 10, 1000, nullptr, OPT_SAVE},

    /* 終端 */
    {0, nullptr, X_INV, 0, 0, 0, nullptr, nullptr},
};
...
static void destroy_sound_device();
...
void osd_update_video_and_audio(void) { /* DO NOTHING */ }

void osd_sound_enable(int enable) { /* DO NOTHING */ }
 ...
 *
 *****************************************************************************/
void osd_set_mastervolume(int attenuation) { /* waveOutSetVolume */ }
...
static void CALLBACK waveOutProc(HWAVEOUT hwo, UINT msg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2);
#endif
...
static int create_sound_device(int stereo) {
  ...
  if (!err) {
    ...
    if (!(sample.data = (unsigned char *)calloc(1, sample.dataSize))) {
      err = true;
    }
  }
  ...
  if (verbose_proc)
    printf("  waveOutOpen=16bit, %s, %dHz : buf-size=%d\n", (stereo) ? "stereo" : "mono", f.nSamplesPerSec,
           bufsize / byte_per_sample);

  /* WAVバッファ (今は無音) を出力 */
  for (i = 0; i < BUFFER_NUM; i++) {
    if (waveOutPrepareHeader(hWaveOut, &whdr[i], sizeof(WAVEHDR)) == MMSYSERR_NOERROR) {
 ...
 */
void wave_event_open(HWAVEOUT hwo) { /* printf("S:open\n"); */ }
...
void wave_event_done(HWAVEOUT hwo, LPWAVEHDR lpwhdr) {
  if (lpwhdr->dwUser) { /* true = Stopped, Never Write */
    ...
    if (waveOutPrepareHeader(hWaveOut, lpwhdr, sizeof(WAVEHDR)) == MMSYSERR_NOERROR) {
 ...
 */
void wave_event_close(HWAVEOUT hwo) { /* printf("S:close\n"); */ }
