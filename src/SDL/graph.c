...
const T_GRAPH_SPEC *graph_init(void) {
  ...
  if (sdl_byte_per_pixel == 2 || sdl_byte_per_pixel == 4) {
    /* OK */;
  } else {
    sdl_depth = 16;
...
const T_GRAPH_INFO *graph_setup(int width, int height, int fullscreen, double aspect) {
  ...
  if (graph_exist) {
    ...
    if ((graph_info.fullscreen == FALSE && fullscreen == FALSE) && (!(sdl_display->flags & SDL_FULLSCREEN))) {
  ...
  sdl_offscreen = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, sdl_depth, 0, 0, 0, 0);
  if (verbose_proc)
  ...
  if (verbose_proc)
    printf("    VideoMode %dx%d -> %dx%dx%d(%d)  %c%c%c%c  R:%x G:%x B:%x\n", width, height, sdl_display->w,
           sdl_display->h, sdl_display->format->BitsPerPixel, sdl_display->format->BytesPerPixel,
           (sdl_display->flags & SDL_SWSURFACE) ? 'S' : '-', (sdl_display->flags & SDL_HWSURFACE) ? 'H' : 'S',
           (sdl_display->flags & SDL_DOUBLEBUF) ? 'D' : '-', (sdl_display->flags & SDL_FULLSCREEN) ? 'F' : '-',
           sdl_display->format->Rmask, sdl_display->format->Gmask, sdl_display->format->Bmask);
...
static int search_mode(int w, int h, double aspect) {
  ...
  for (i = 0; sdl_mode[i]; i++) {
    /* 画面サイズに収まっていること */
    if (w <= sdl_mode[i]->w && h <= sdl_mode[i]->h) {
      ...
      if (fit == -1) {
        ...
        if (aspect >= 1.0 || aspect < 0.01) {
          ...
          if (((tmp_h - h) < (fit_h - h)) || ((tmp_h == fit_h) && (tmp_a < fit_a))) {
            fit = i;
            fit_w = tmp_w;
            fit_h = tmp_h;
            fit_a = tmp_a;
          ...
          if (((tmp_w - w) < (fit_w - w)) || ((tmp_w == fit_w) && (tmp_a < fit_a))) {
            fit = i;
            fit_w = tmp_w;
            fit_h = tmp_h;
            fit_a = tmp_a;
...
void graph_set_window_title(const char *title) { SDL_WM_SetCaption(title, title); }
