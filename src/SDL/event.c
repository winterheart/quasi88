...
static const int keysym2key88_default[SDLK_LAST] = {
    ...
    0, 0, KEY88_HOME,                   /*  SDLK_CLEAR      = 12,   */
    KEY88_RETURNL,                      /*  SDLK_RETURN     = 13,   */
    0, 0, 0, 0, 0, KEY88_STOP,          /*  SDLK_PAUSE      = 19,   */
    0, 0, 0, 0, 0, 0, 0, KEY88_ESC,     /*  SDLK_ESCAPE     = 27,   */
    0, 0, 0, 0, KEY88_SPACE,            /*  SDLK_SPACE      = 32,   */
  ...
  while (SDL_PeepEvents(&E, 1, SDL_GETEVENT,
    ...
    case SDL_KEYUP:
      ...
      if (now_unicode == FALSE) { /* キーASCII時 */
        ...
        if (E.key.keysym.scancode < COUNTOF(scancode2key88) && scancode2key88[E.key.keysym.scancode] >= 0) {
...
static const char *identify_callback(const char *parm1, const char *parm2, const char *parm3) {
  if (my_strcmp(parm1, "[SDL]") == 0) {
    if (parm2 == NULL || my_strcmp(parm2, video_driver) == 0) {
      return NULL; /* 有効 */
    }
  }
