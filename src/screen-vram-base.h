...
#define make_mask_mono(mask)                                                                                           \
  do {                                                                                                                 \
    mask = 0xffffffff;                                                                                                 \
    if (grph_pile & GRPH_PILE_BLUE)                                                                                    \
      mask &= 0x00ffff00;                                                                                              \
    if (grph_pile & GRPH_PILE_RED)                                                                                     \
      mask &= 0x00ff00ff;                                                                                              \
    if (grph_pile & GRPH_PILE_GREEN)                                                                                   \
      mask &= 0x0000ffff;                                                                                              \
  } while (0)
#define get_pixel_mono(data, col) (TYPE)(((data)&0x00808080) ? (col) : COLOR_PIXEL(0))
#else
#define make_mask_mono(mask)                                                                                           \
  do {                                                                                                                 \
    mask = 0xffffffff;                                                                                                 \
...
#define get_pixel_mono(data, col) (TYPE)(((data)&0x80808000) ? (col) : COLOR_PIXEL(0))
#endif

/*----------------------------------------------------------------------*/

#ifdef LSB_FIRST
#define get_pixel_400_B(data, col) (TYPE)(((data)&0x00000080) ? (col) : COLOR_PIXEL(0))
#define get_pixel_400_R(data, col) (TYPE)(((data)&0x00008000) ? (col) : COLOR_PIXEL(0))
#else
#define get_pixel_400_B(data, col) (TYPE)(((data)&0x80000000) ? (col) : COLOR_PIXEL(0))
#define get_pixel_400_R(data, col) (TYPE)(((data)&0x00800000) ? (col) : COLOR_PIXEL(0))
#endif
